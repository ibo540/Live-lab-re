                    ...
                    <div className="rounded-xl border border-white/10 overflow-hidden bg-slate-900/50 shadow-2xl">
                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-sm">
                                <thead className="bg-slate-950/80 text-slate-400 uppercase tracking-wider text-xs font-semibold">
                                    <tr>
                                        <th className="p-4 border-b border-white/10">Combination #</th>
                                        <th className="p-4 border-b border-white/10">A (Tuition Increase)</th>
                                        <th className="p-4 border-b border-white/10">B (Student Mobilization)</th>
                                        <th className="p-4 border-b border-white/10">C (Trust in Administration)</th>
                                        <th className="p-4 border-b border-white/10 text-right">NO PROTEST (cases)</th>
                                        <th className="p-4 border-b border-white/10 text-right">PROTEST (cases)</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-white/5">
                                    {method.cases.map((c) => (
                                        <tr key={c.id} className="hover:bg-white/5 transition-colors">
                                            <td className="p-4 font-medium text-slate-300">{c.label}</td>
                                            <td className="p-4 text-slate-300">{c.conditions['A (Tuition Increase)'] ? 'Yes' : 'No'}</td>
                                            <td className="p-4 text-slate-300">{c.conditions['B (Student Mobilization)'] ? 'Yes' : 'No'}</td>
                                            <td className="p-4 text-slate-300">{c.conditions['C (Trust in Administration)'] ? 'Yes' : 'No'}</td>
                                            <td className="p-4 text-right font-mono text-slate-400">{c.stats?.noProtest ?? 0}</td>
                                            <td className="p-4 text-right font-mono text-white font-bold">{c.stats?.protest ?? 0}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
